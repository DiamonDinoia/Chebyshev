  ...
  public:
            ...
            for (long axis = 0; axis < N; ++axis) {
                ...
                for (long i = 0; i < total_elements_in_slice; ++i) {
                    ...
                    for (long j = 0; j < m; ++j) {
                        ...
                        element_idx[axis] = j;
                        line_data[j] =
                            std::apply([&](auto &&...args) { return coeffs_(std::forward<decltype(args)>(args)...); },
                                       element_idx);
                    }
                    ...
                    for (long k = 0; k < m; ++k) {
                        ...
                        element_idx[axis] = k;
                        std::apply(
                            [&](auto &&...args) { coeffs_(std::forward<decltype(args)>(args)...) = dct_data[k]; },
                            element_idx);
                    }
    ...
    vecM_t operator()(const vecN_t &x) const {
        ...
        for (long i = 0; i < total_coeffs; ++i) {
            ...
            // Add the term C[k_0, ..., k_{N-1}, o] * prod_{d=0}^{N-1} T_{k_d}(x_d) to the result for each output
            // component 'o'.
            for (long o = 0; o < M; ++o) {
                for (long d = 0; d < N; ++d)
                ...
                auto coeff_value = std::apply(
                    [&](auto &&...args) { return coeffs_(std::forward<decltype(args)>(args)...); }, full_coeff_idx);
    ...
    template <size_t D_idx> void sample(F &f, std::array<long, N> &idx) {
        for (long i = 0; i < shape_[D_idx]; ++i) {
            ...
            if constexpr (D_idx + 1 < N) {
                ...
                // Store the sampled values in the coefficients tensor at the corresponding index (j_0, ..., j_{N-1},
                // o).
                for (long o = 0; o < M; ++o) {
