...
int main() {
    ...
    alignas(64) std::array<std::complex<double>, num_points> output{};
    std::vector<double> unaligned_random_points(num_points);
    std::vector<std::complex<double>> unaligned_output(num_points);
    std::vector<double, xsimd::aligned_allocator<double, 64>> aligned_random_points(num_points);
    std::vector<std::complex<double>, xsimd::aligned_allocator<std::complex<double>, 64>> aligned_output(num_points);

    // Essential constexpr declarations (placed just before their dependent uses)
    constexpr auto f = [](const double x) { return std::complex(std::cos(x), std::sin(x)); };
    ...
    if (run_non_constexpr_benchmarks) {
        ...
        const auto sum_unaligned_auto =
            std::accumulate(unaligned_output.begin(), unaligned_output.end(), std::complex<double>(0.0, 0.0));
        ...
        const auto sum_unaligned_manual =
            std::accumulate(unaligned_output.begin(), unaligned_output.end(), std::complex<double>(0.0, 0.0));
        ...
        const auto sum_aligned_auto =
            std::accumulate(aligned_output.begin(), aligned_output.end(), std::complex<double>(0.0, 0.0));
    ...
    if (run_constexpr_benchmarks) {
        ...
        const auto sum_unaligned_auto_const =
            std::accumulate(unaligned_output.begin(), unaligned_output.end(), std::complex<double>(0.0, 0.0));
        ...
        const auto sum_unaligned_manual_const =
            std::accumulate(unaligned_output.begin(), unaligned_output.end(), std::complex<double>(0.0, 0.0));
        ...
        const auto sum_aligned_auto_const =
            std::accumulate(aligned_output.begin(), aligned_output.end(), std::complex<double>(0.0, 0.0));
        ...
        const auto sum_aligned_manual_const =
            std::accumulate(aligned_output.begin(), aligned_output.end(), std::complex<double>(0.0, 0.0));
