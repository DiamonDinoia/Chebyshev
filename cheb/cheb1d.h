...
template <class Func> class BarCheb1D {
  ...
  private:
    ...
    constexpr double map_from_domain(double x) const { return (2.0 * x - (b + a)) / (b - a); }
...
inline double map_to_domain(double x, double a, double b) { return 0.5 * ((b - a) * x + (b + a)); }

inline double map_from_domain(double x, double a, double b) { return (2.0 * x - (b + a)) / (b - a); }
 ...
 * ────────────────────────────────────────────────────────────────*/
template <class Func> class Mon1D {
  public:
    Mon1D(Func F, int n, double a = -1.0, double b = 1.0)
        : N(n), low(b - a), hi(b + a), cheb(n), mono(n, 0) // ← mono initialised to zeros
    {
...
template <class Func> class Hor1D : public Mon1D<Func> {
  public:
...
template <class Func> class FixedHor : public Mon1D<Func> {
  public:
  ...
  private:
    ...
    template <std::size_t idx> __always_inline static constexpr double horner_step(const double *c, double const x) {
        if constexpr (idx == 0) {
            return c[0];
        } else {
    ...
    template <std::size_t N> __always_inline static double eval_fixed(const double *c, double x) {
        if constexpr (N == 0) {
            return 0.0;
        } else {
    ...
    template <std::size_t... Is> static constexpr auto make_table(std::index_sequence<Is...>) {
    ...
    double dispatch_eval(const double *c, std::size_t n, double x) const {
        alignas(32) static constexpr auto table = make_table(std::make_index_sequence<DEFAULT_MAX_COEFFS + 1>{});
...
template <class Func> class FixedEst : public Mon1D<Func> {
  public:
  ...
  private:
    ...
    template <std::size_t N> __always_inline static constexpr std::size_t ct_log2() {
    ...
    template <std::size_t K> __always_inline static constexpr double compute_x_power_of_two(double x) {
        if constexpr (K == 0) {
            return x;
        } else {
    ...
    __always_inline static constexpr auto make_powers_tuple(double x, std::index_sequence<K...>) {
    ...
    __always_inline static constexpr double estrin_eval_recursive(double x, PowersTuple powers, const double *coeffs) {
        if constexpr (Count == 0) {
            ...
            const double high = estrin_eval_recursive<PowersTuple, Count - split>(x, powers, coeffs + split);
    ...
    __always_inline static constexpr double estrin_eval(double x, const double *coeffs) {
...
template <class Func> class Est1D : public Mon1D<Func> {
  public:
    using Mon1D<Func>::Mon1D; // inherit constructor
    /* Horner evaluation on [-1,1] then un-map */
    double operator()(const double pt) const {
...
template <class Func> class OptHor : public Mon1D<Func> {
  public:
    OptHor(Func F, int n, double a = -1.0, double b = 1.0) : Mon1D<Func>(F, n, a, b) {
        // reverse to have c[0] first
        std::reverse(this->mono.begin(), this->mono.end());
    }
  ...
  private:
    ...
    __always_inline static constexpr double horner_step(const double *c, double x) {
        if constexpr (I == 0)
            return c[0];
        else
            return std::fma(horner_step<N, I - 1>(c, x), x, c[I]);
    }

    template <std::size_t N> __always_inline static constexpr double eval_scalar(const double *c, double x) {
        if constexpr (N == 0)
            return 0.0;
        else
    ...
    __always_inline static double eval_block4(const double *c, double x) // c points at c0
    {
    ...
    double dispatch_eval(const double *c, std::size_t n, double x) const {
        static constexpr auto tbl = make_table(std::make_index_sequence<MAX_COEFFS + 1>{});
