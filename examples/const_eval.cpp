...
int main() {
    ...
    std::cout << "  Evaluation time (single point): " << std::fixed << std::setprecision(9)
              << eval_duration_1.count() * 1e6 << " us\n";
    ...
    std::cout << "  Evaluation time (single point): " << std::fixed << std::setprecision(9)
              << eval_duration_3.count() * 1e6 << " us\n";

    // -------------------------------------------------------------------------
    // 4. Runtime Epsilon (C++17 API)
    //    API: poly_eval::make_func_eval<MaxN_val, NumEvalPoints_val, Iters_compile_time>(Func F, double eps, InputType
    //    a, InputType b)
    // -------------------------------------------------------------------------
    ...
    auto poly_error_driven_runtime_eps =
        poly_eval::make_func_eval<max_n_runtime_c17, eval_points_runtime_c17, iters_runtime_c17>(
    ...
    std::cout << "  Evaluation time (single point): " << std::fixed << std::setprecision(9)
              << eval_duration_4.count() * 1e6 << " us\n";

    // -------------------------------------------------------------------------
    // 5. Compile-Time Epsilon (C++20 API - Still runtime fitting due to FuncEval internal)
    //    API: poly_eval::make_func_eval<eps_val, MaxN_val, NumEvalPoints_val, Iters_compile_time>(Func F, InputType a,
    //    InputType b)
    // -------------------------------------------------------------------------
    ...
    auto poly_error_driven_compile_time_eps =
        poly_eval::make_func_eval<COMPILE_TIME_EPS, COMPILE_TIME_MAX_N, COMPILE_TIME_EVAL_POINTS, COMPILE_TIME_ITERS>(
    ...
    //    API: poly_eval::make_constexpr_fixed_degree_eval<N_DEGREE, Iters_compile_time>(Func F, InputType a, InputType
    //    b)
    // -------------------------------------------------------------------------
#if __cplusplus >= 202002L
    ...
    std::cout << "Full Compile-Time (double, Fixed N=" << CONSTEXPR_DEGREE << ", Iters=" << CONSTEXPR_ITERS
              << " - C++20+):\n";
    std::cout << "  Fitting time: 0.000000 ms (performed at compile-time)\n";
    constexpr auto poly_constexpr_eval =
        poly_eval::make_func_eval<CONSTEXPR_DEGREE, CONSTEXPR_ITERS>(my_func_constexpr, domain_a6, domain_b6);
    ...
    std::cout << "  Evaluation time (single point, runtime): " << std::fixed << std::setprecision(9)
              << eval_duration_6.count() * 1e6 << " us\n";
